...
import { getConfigFilePath,getBuyerPuclicKeyPath, getHodlerFilePath, getPublicKeyFilePath, getRevalidationSuccessPath, getSalesFilePath, getSalesTrackerLockPath, getSalesTrackerSuccessPath } from './verify/paths'
...
import {getBOTTOKEN, getBOTCLIENTID} from './constant'
...
app.post('/createProject', async (req: any, res: Response) => {
  ...
  var newProjectConfig = {
    ...
    discord_client_id: validateRequired("discord_client_id", getBOTCLIENTID()),
    ...
    discord_bot_token: validateRequired("discord_bot_token", getBOTTOKEN()),
...
app.post('/updateProject', async (req: any, res: Response) => {
  ...
  if (getBOTCLIENTID()) {
    config.discord_client_id = getBOTCLIENTID()
  }
  ...
  if (getBOTTOKEN() != defaultRedactedString) {
    config.discord_bot_token = getBOTTOKEN()
  }
...
app.post('/beBuyer', async (req: any, res: Response) => {
  ...
  var isSuccessful = await write(getBuyerPuclicKeyPath(publicKeyString), JSON.stringify({
  ...
  logger.info(`successfully bought feature verify tool for ${publicKeyString}`)
  return isSuccessful
})

// search if isBuyer

app.post('/getBuyerPub', async (req: any, res: Response) => {
  ...
  var userProject = JSON.parse(await read(getBuyerPuclicKeyPath(publicKeyString)))
  if(userProject){
    return userProject.PubKey
  }
  else {
    return res.sendStatus(400)
  } 
